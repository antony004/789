...
namespace _789.Areas.Identity.Pages.Account
{
    ...
    {
        private readonly SignInManager<CustomIdentityModel> _signInManager;
        ...
        public async Task<IActionResult> OnGetAsync(string returnUrl = null)
        {
            ...
            try
            {
                var routeData = Url.ActionContext.HttpContext.Request.Query[Url.ActionContext.HttpContext.Request.Query.Keys.ToList()[0]];
                string refcode = routeData.ToString();
                await _servicer.VerifyReferralCode(refcode);
                Code = refcode;
            }
            catch (Exception ex)
            {
                return RedirectToPage("ReferralCode");
            }
            ReturnUrl = returnUrl;
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    string refcode = Code;
                    await _servicer.VerifyReferralCode(refcode, true);
        ...
        private IUserEmailStore<CustomIdentityModel> GetEmailStore()
        {
            if (!_userManager.SupportsUserEmail)
            {
                throw new NotSupportedException("The default UI requires a user store with email support.");
            }
            return (IUserEmailStore<CustomIdentityModel>)_userStore;
        }
    }
}
